...
class QueryGraphState(TypedDict):
    ...
    # 辅助信息
    item_names: List[str]  # 大模型提取出的商品名称，不一定在数据库里有的
    rewritten_query: str  # 改写后的问题
    is_history: bool  # True表示拿去的答案是历史记录的
    is_stream: bool  # 是否流式输出标记
    image_urls: List[str]  # 答案中引用的图片链接

    confirmed_item_name_list:list # 已确认item_names列表
    # options_item_name_list:list # 可选择的item_names列表

    is_web_only: bool # 是否仅开启网络搜索
