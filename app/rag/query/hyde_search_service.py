...
def get_data_and_validates(state:QueryGraphState) -> tuple[str,list[str]]:
    ...
    if not rewritten_query or len(confirmed_item_name_list) == 0:
        logger.error(f"重写问题或者关联的主体为空,无法继续业务!")
        raise ValueError(f"重写问题或者关联的主体为空,无法继续业务!")

    return rewritten_query,confirmed_item_name_list

@step_log("milvus_search_hyde_entity")
def milvus_search_hyde_entity(hyde_answer:str,rewritten_query:str, confirmed_item_name_list:list[str]):
    """
      使用重写问题,对向量库进行搜索!
...
def call_llm_by_rewritten_query(rewritten_query,confirmed_item_name_list) -> str:
    """
      调用模型生成假设性答案
    :param rewritten_query:
    :return:
    ...
    # 2. 加载和封装提示词
    prompt_text =  load_prompt("hyde_prompt",rewritten_query = rewritten_query,item_names_str =",".join(confirmed_item_name_list))
    messages = [
...
def search_by_hyde(state: QueryGraphState):
    ...
    """
    # 1. 参数获取和校验
    rewritten_query, confirmed_item_name_list = get_data_and_validates(state)
    # 2. 根据问题获取答案(lm)
    hyde_answer = call_llm_by_rewritten_query(rewritten_query,confirmed_item_name_list)

    # 2. 进行向量库混合内容检索
    milvus_response = milvus_search_hyde_entity(hyde_answer,rewritten_query,confirmed_item_name_list)
    # 3. 进行数据格式化处理
