...
def milvus_search_entity(rewritten_query, confirmed_item_name_list):
    """
      使用重写问题,对向量库进行搜索!
      注意: 需要添加item_name的过滤条件
    :param rewritten_query:
    :param confirmed_item_name_list:
    :return: 返回处理一层后的列表
    """
    # 1. 重写问题生成向量
...
def search_by_embedding(state: QueryGraphState):
    ...
    # 2. 进行向量库混合内容检索
    milvus_response = milvus_search_entity(rewritten_query,confirmed_item_name_list)
    # 3. 进行数据格式化处理
