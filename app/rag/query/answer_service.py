...
from app.shared.clients.mongo_history_utils import save_chat_history_message
...
def check_state_has_answer(state):
    ...
    is_web_only = state.get("is_web_only",False)
    answer = state.get("answer")

    if not answer:
        # answer没值：要么是正常流程，要么仅是网络搜索
        logger.info(f"没有answer,证明有明确的item_names，但不一定是在库里有的主体，正常返回结果!!")
        return False
    else:
        # 这里表示有answer有两种种情况：
        # 1.直接读取的历史记录
...
def get_data_and_validates(state):
    ...
    if is_web_only:
       if  len(reranked_docs) == 0 or not rewritten_query or  len(item_names) == 0 :
       ...
       return reranked_docs, history, item_names, rewritten_query
        ...
        if len(reranked_docs) == 0 or len(confirmed_item_name_list) == 0 or not rewritten_query:
            logger.info(f"没有reranker_docs,confirmed_item_name_list,rewritten_query,请检查参数!!")
            raise ValueError("没有reranker_docs,confirmed_item_name_list,rewritten_query,请检查参数!!")

        return reranked_docs,history,confirmed_item_name_list,rewritten_query


def load_prompt_text(reranker_docs, history, confirmed_item_name_list, rewritten_query) -> str:
    """
    加载提示词文件! 拼接提示词!
    ...
    # 已确认item_names关联
    item_names_text = ",".join(confirmed_item_name_list)
...
def call_llm_generate(answer_prompt_text, state):
    ...
    is_web_only = state.get("is_web_only", False)
    if is_web_only:
        final_answer = r"您问的问题不在我们的知识库中，我们从网络上查询到的结果如下：\n" + final_answer
...
def save_history_message(state):
    history_repository.save_message(
        ...
        rewritten_query=state.get("rewritten_query"),
        item_names=state.get("item_names",[]) if state.get("is_web_only",False) else state.get("confirmed_item_name_list",[]),
        image_urls=state.get("image_urls",[])
    )
    # 永久历史记录
    save_chat_history_message(
...
def generate_answer(state: QueryGraphState) -> QueryGraphState:
    ...
    if not has_answer:
        # 3. 没有结果,获取并且校验参数
        reranker_docs,history,confirmed_item_name_list,rewritten_query = get_data_and_validates(state)
        # 4. 拼接提示词的上下文,加载外部的提示词文件
        answer_prompt_text = load_prompt_text(reranker_docs,history,confirmed_item_name_list,rewritten_query)
