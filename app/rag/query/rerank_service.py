...
def is_web_only_validate(state):
    web_search_docs = state.get("web_search_docs", [])
    rewritten_query = state.get("rewritten_query")
    # 仅网络搜索
    if not web_search_docs:
        logger.error(f"关键参数:{web_search_docs}为空,业务无法继续进行!!")
        raise ValueError(f"关键参数:{web_search_docs}为空,业务无法继续进行!!")
    if not rewritten_query:
        logger.error(f"关键参数:{rewritten_query}为空,业务无法继续进行!!")
        raise ValueError(f"关键参数:{rewritten_query}为空,业务无法继续进行!!")

    return  web_search_docs, rewritten_query
...
def deal_chunk_list(rrf_chunks, web_search_docs,is_web_only):
    """
    进行两路数据融合
...
def rerank_documents(state: QueryGraphState) -> list[dict]:
    ...
    if is_web_only:
        web_search_docs,rewritten_query  = is_web_only_validate(state)
        rrf_chunks = []
    else:
        rrf_chunks,web_search_docs,rewritten_query = get_rewritten_query_and_validate(state)

    # 2. 多路数据融合格式统一
    final_chunk_list = deal_chunk_list(rrf_chunks,web_search_docs,is_web_only)
